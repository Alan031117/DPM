...
class Encoder(nn.Module):
    ...
    def forward(self, points: Tensor, points_padding: Tensor) -> List[Tensor]:
        """FBH
            将输入点云的坐标和特征分开处理
                l0_coor：所有点云初始坐标 (B, 3, N)
                l0_fea：所有初始点云的所有特征 (B, 3, N)
                l0_padding：所有点云的填充掩码 (B, N) ，True为填充点，False为正常点
        """
        ...
        recorder = [[l0_coor, l0_fea, l0_padding]]  # 记录第一层处理后的坐标、特征和掩码

        # 下采样（剔除数据）
        for layer in self.downsampler:
            new_coor, new_fea, new_padding = layer(*recorder[-1])   # 将recorder[-1]解包后输入当前下采样层
            recorder.append([new_coor, new_fea, new_padding])   # 记录新一层的坐标、特征和填充掩码
        ...
        return recorder[-1]  # 返回最终上采样后的坐标、特征和掩码
