...
class SlamDatasets(Dataset):  # 定义SlamDatasets类，继承自PyTorch的Dataset类，用于SLAM任务的数据集数据读取
    ...
    def _map_query(self, index: int, K: int, info: dict) -> List:
        ...
        offset = index - self.pcd_range[dataset_id]     # 获取 index 在数据集中的偏移量
        curren_dataset = self.dataset_list[dataset_id]      # 当前数据集 = index所在数据集
        scene_id, frame_offset = curren_dataset.get_frame_order(offset)     # 获取 场景ID 和 帧偏移量
        frame_dis = self.frame_distance[dataset_id][scene_id][frame_offset]     # 获取index帧和其他帧的距离
        ...
        if 'carla' in curren_dataset.name.lower():
            ...
            refined_SE3_file = os.path.join(curren_dataset.scene_list[scene_id].root,
                                            'refined_SE3.pkl')
        ...
        for map_frame_offset in map_frame_offsets:
            frame = curren_dataset[offset + map_frame_offset]  # 加载对应帧数据
