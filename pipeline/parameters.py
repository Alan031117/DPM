...
parser.add_argument('--yaml_file', '-yaml',     default='',                     type=str,
                    help='Yaml file which contains config parameters')  # 添加yaml_file参数，默认值为空字符串，类型为字符串，表示包含配置参数的yaml文件路径
parser.add_argument('--num_workers',            default=4,                      type=int,
                    help='Number of threads used for parallel data loading')  # 添加num_workers参数，默认值为4，类型为整数，表示用于并行数据加载的线程数
parser.add_argument('--thread_safety',          default=False,                  action='store_true',
                    help='Whether the data loading method is thread safety')  # 添加thread_safety参数，默认值为False，表示数据加载方法是否线程安全
parser.add_argument('--use_cuda',               default='True',                 type=str_to_bool,
                    help='Using cuda to accelerate calculations')  # 添加use_cuda参数，默认值为'True'，类型为字符串（通过str_to_bool转换为布尔值），表示是否使用CUDA加速计算
...
parser.add_argument('--infer_src',              default=[],                     type=list,
                    help='Sequential pcd data director list for inference')  # 添加infer_src参数，默认值为空列表，类型为列表，表示用于推理的连续PCD数据目录列表
...
parser.add_argument('--multi_thread', '-mt',    default=False,                  action='store_true',
                    help='Using multi-thread asynchronous pipeline to accelerating inference')  # 添加multi_thread参数，默认值为False，表示是否使用多线程异步流水线加速推理
