...
class DeepPointModelPipeline(nn.Module):  # 定义一个名为DeepPointModelPipeline的类，继承自PyTorch的nn.Module类，用于封装DeepPoint网络模型的训练流程。
    ...
    def _train_registration(self, pcd: Tensor, R: Tensor, T: Tensor, padding_mask: Tensor, calib: Tensor, info: dict)\
            -> Tuple[Tensor, dict]:  # 定义方法_train_registration，用于训练配准模块，返回损失和评价指标字典。
        """
        :param pcd: (S, 4, N)  # 点云数据，形状为(S, 4, N)S表示帧数，（ 前三个表示 xyz ，最后一个表示 label ），N表示点的数量。
        ...
        """
        coor, fea, mask = self.encoder(pcd, padding_mask)  # 使用 encoder 提取 pcd 的特征点(坐标、特征、掩码)
        """FBH
        :param coor:最终上采样后每个点的坐标(S, 3, N)
        :param fea:最终上采样后每个点的特征(S, ???, N)
        :param padding_mask:最终上采样后的掩码(3, 原始点云一帧总点数)
        """
        S, _, N = coor.shape    # 获取coor的形状，S表示所有帧的数量，N表示一张局部地图里点的数量
        B = info['num_map']     # 获取num_map的值，B表示选取的局部地图数量
        S = S // B      # S变为一张局部地图中的帧数
        coor = coor * self.args.slam_system.coor_scale  # 对坐标进行缩放，以匹配SLAM系统的尺度，尺度系数源于YAML
        pcd_index = np.asarray([dsf_index[2] for dsf_index in info['dsf_index']])  # 提取每个帧索引
        ...
        if S <= map_size_max:  # 如果帧数不超过最大局部地图尺寸
            if random.random() < 0.5:  # 以50%的概率选择1或随机拆分点
                ...
                S1 = random.randint(1, S - 1)  # 随机拆分帧数，确保地图帧数不超过最大局部地图尺寸
        else:
            S1 = random.randint(S - map_size_max, map_size_max)  # 如果超过最大局部地图尺寸，确保拆分后的帧数不超过
        ...
        """
            ...
            :param R = {Tensor:(16, 3, 3)}  旋转矩阵
            :param T = {Tensor:(16, 3, 1)}  平移矩阵
        """
